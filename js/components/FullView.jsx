        ...
        <div className="fv-buses">
          {(function(){
            ...
            return list.map(function(bus){
              ...
              const seats = Array.from({length: bus.cap}, function(_, i){ return i < effectiveFil; });
              const cfgMap = {
              ...
              return (
                <div key={bus.id} className={'fv-bus-row ' + cf.cls}>
                  ...
                  <div className="fv-bus-seats">
                    {seats.map(function(on, i){ return <div key={i} className="fv-bus-seat" style={{background: on ? cf.on : cf.off, border: on ? 'none' : '1px solid ' + cf.bo}}/>; })}
                  </div>
                  <div className="fv-bus-status">
                    ...
                    {st.t === 'crit' && (
                      <div>
                        ...
                        <div className="fv-bus-status-crit-num">Sisa <span className="fv-bus-status-crit-big">{st.r}</span> seat!</div>
                      </div>
                    )}
                    {st.t === 'low' && (
                      <div>
                        <div className="fv-bus-status-low-label">Sisa Seat</div>
                        <div className="fv-bus-status-low-num">{st.r}<span className="fv-bus-status-low-unit"> kursi</span></div>
                      </div>
                    )}
                    {st.t === 'ok' && (
                      <div>
                        <div className="fv-bus-status-ok-label">Tersedia</div>
                        <div className="fv-bus-status-ok-num">{st.r}<span className="fv-bus-status-ok-unit"> kursi</span></div>
                      </div>
                    )}
                  </div>
                  {editMode && (
